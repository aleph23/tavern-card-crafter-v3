...
const AlternateGreetings = ({
  ...
  const handleAIGenerateGreeting = async () => {
    ...
    try {
      const prompt = generateAlternateGreeting(characterData as unknown as UsedCharacterData)
  ...
  const handleClearAll = () => {
    ...
    toast({ title: 'Cleared', description: 'All alternative greetings have been cleared' })
  }
            ...
            {editingIndex === index ? (
              ...
              <>
                <div className='absolute top-2 right-2 flex gap-1'>
                  <Button size='sm' variant='ghost' onClick={() => startEditing(index)} className='h-8 w-8 p-0'>
                    <Edit2 className='w-4 h-4' />
                  </Button>
                  <Button size='sm' variant='ghost' onClick={() => removeGreeting(index)} className='h-8 w-8 p-0'>
                    <X className='w-4 h-4' />
                  </Button>
                </div>
