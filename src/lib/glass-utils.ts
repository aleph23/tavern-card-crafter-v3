...
export const glassVariants = cva('', {
  variants: {
    variant: {
      glass: '', // Base styling is handled by individual components
      frosted: 'backdrop-blur-xl bg-white/5 border border-white/20 shadow-lg',
      fluted:
        'backdrop-blur-md bg-white/5 border border-white/10 bg-[linear-gradient(90deg,rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[size:10px_100%] shadow-md',
      crystal: 'backdrop-blur-sm bg-white/5 border border-white/30 shadow-[inset_0_1px_1px_rgba(255,255,255,0.4)]',
    },
  },
  defaultVariants: {
    variant: 'glass',
...
export function getGlassCSSVars(customization?: GlassCustomization): Record<string, string> {
  ...
  if (customization.outlineWidth !== undefined) {
    const width =
      typeof customization.outlineWidth === 'number' ? `${customization.outlineWidth}px` : customization.outlineWidth
